...
document.querySelector("#quizForm").addEventListener("submit", function (event) {
    ...
    /*
        for(let j=1; j<preguntas.length; j++){
            if (respuesta[j] == "") { //Sin contestar
                console.log('No has contestado la pregunta '+(j+1)+'\n');
                errores += 'No has contestado la pregunta '+(j+1)+'\n';
            } else if (respuesta[j] == respuestas.ok[j]) {
                console.log('Has acertado la pregunta '+(j+1)+'\n');
                aciertos++;
            } else {
                console.log('Fallaste Burlan Caster!!\n');
                errores += 'Fallaste la pregunta '+(j+1)+'\n';
            }
        }
    */
    console.log(errores);
